...
use Mufuphlex\Tests\Cache\Dummies\GuardDummy;
...
class GuardAbstractTest extends \PHPUnit_Framework_TestCase
{
    public function testSetCache()
    {
        ...
        static::assertSame($guard, $guard->setCache($this->getCache()));
    }
    ...
    {
        $guard = new GuardDummy();
        static::assertSame($guard, $guard->setCleanupStrategy($this->getCleanupStrategy()));
    }
    ...
    {
        return static::createMock('\Mufuphlex\Cache\CleanupStrategyInterface');
    }
    ...
    {
        $strategy = static::createMock('\Mufuphlex\Cache\CheckStrategyInterface');
        $strategy
            ->expects(static::once())
            ->method('check')
            ->willReturn($return);
